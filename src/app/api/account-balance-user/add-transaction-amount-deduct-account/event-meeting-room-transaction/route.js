...
import { SUCCESSFUL_AMOUNT_DEDUCT_EVENT_MEETING_ROOM_TRANSACTION } from "@/constant string files/apiSuccessMessageConstants.js";
...
async function POST(NextRequest){
    try{
        ...
        const eventMeetingRoomBookingAmount = transactionInfo.deductionAmount;

        const hotelUser = await HotelCustomersUsers.findById(customerId);

        if(hotelUser){
            const transactionAmount = eventMeetingRoomBookingAmount; 
            const transactionType = 'Money Deducted From Account';
            const transactionDescription = 'Money Deducted From Account By Event/ Meeting Room Booking';
            const transactionDateTime = new Date();
            const beforeAccountBalance = hotelUser.accountBalance;
            const updatedAccountBalance = Number(beforeAccountBalance) - Number(eventMeetingRoomBookingAmount);
            ...
            return NextResponse.json(
                { message: SUCCESSFUL_AMOUNT_DEDUCT_EVENT_MEETING_ROOM_TRANSACTION, transactionId },
                { status: 200 }
            );
        }
        else{
        ...
        console.log('src/app/api/account-balance-user/add-transaction-amount-deduct-account/event-meeting-room-transaction/route.js')
        console.log(error);
        return NextResponse.json(
