...
import { SUCCESSFUL_AMOUNT_DEDUCT_DINING_TRANSACTION } from "@/constant string files/apiSuccessMessageConstants.js";
...
async function POST(NextRequest){
    try{
        ...
        const diningBookingAmount = transactionInfo.deductionAmount;

        const hotelUser = await HotelCustomersUsers.findById(customerId);

        if(hotelUser){
            const transactionAmount = diningBookingAmount; 
            const transactionType = 'Money Deducted From Account';
            const transactionDescription = 'Money Deducted From Account By Dining Booking';
            const transactionDateTime = new Date();
            const beforeAccountBalance = hotelUser.accountBalance;
            const updatedAccountBalance = Number(beforeAccountBalance) - Number(diningBookingAmount);
            ...
            return NextResponse.json(
                { message: SUCCESSFUL_AMOUNT_DEDUCT_DINING_TRANSACTION, transactionId },
                { status: 200 }
            );
        }
        else{
        ...
        console.log('src/app/api/account-balance-user/add-transaction-amount-deduct-account/dining-transaction/route.js')
        console.log(error);
        return NextResponse.json(
